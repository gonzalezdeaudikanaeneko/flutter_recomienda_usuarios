...
class _BookingScreen extends State<BookingScreen> {
  ...
  confirmBooking(BuildContext context) async {
    ...
    for(var i = 0; i < selectedFuncion.slot; i++){
      DocumentReference servicioBooking = selectedServicio.reference.collection('${DateFormat('dd_MM_yy').format(selectedDate)}')
    ...
    batch.commit().then((value) {
      ...
      var notificationPayload = NotificationPayloadModel(
          ...
          notification: NotificationContent(
            title: 'Nueva Reserva',
            body: '${FirebaseAuth.instance.currentUser?.email as String} (${selectedTime.substring(0,5)} - ${DateFormat('dd/MM/yyyy').format(selectedDate)})'
          ),
      );
